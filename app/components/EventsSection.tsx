  ...
  return (
    <section id="events" className="h-[100svh] flex flex-col justify-center items-center overflow-y-auto py-[clamp(1.5rem,5vh,2.5rem)] px-6">
      <div className="max-w-xl mx-auto w-full rounded-2xl px-4 py-[clamp(1.25rem,4vh,2rem)] sm:px-6"
        ...
        <AnimateOnScroll>
          <p className="heading-display text-xs text-[#fff9f3] text-center mb-[clamp(1rem,4vh,1.75rem)]">Event</p>
        </AnimateOnScroll>
          ...
          {events.map(({ name, date, time, venue, address, cardBg, cardBgPosition, googleMapsUrl }, i) => (
            <AnimateOnScroll key={name} delay={0.1 + i * 0.1}>
              <div className="rounded-lg p-4">
                <h3 className="heading-gold heading-display text-center mb-[clamp(0.75rem,3.5vh,1.5rem)]"
                  style={{ fontSize: "clamp(1.1rem, 4.5vw, 1.5rem)", letterSpacing: "0.1em", color: "#fff9f3" }}>
                  {name}
                </h3>
                <div className="space-y-[clamp(0.9rem,4vh,1.75rem)]">
                  {[
                    {
                    ...
                    <div key={label} className={`flex justify-center gap-3 ${sub ? "items-start" : "items-center"}`}>
                      ...
                      <div className="text-center">
                        ...
                        {sub && <p className="text-xs text-[#fff9f3]/60 mt-1">{sub}</p>}
                        {label === "Venue" && (
                          <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer"
                            className="inline-block text-xs text-[#fff9f3] underline mt-2 hover:text-[#fff9f3] transition-colors">
                            Get Directions
                          </a>
                        )}
                      </div>
